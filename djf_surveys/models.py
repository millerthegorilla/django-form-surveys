...
class Survey(BaseModel):
    ...
    editable = models.BooleanField(
        _("editable"), default=True, help_text=_("If False, user can't edit record.")
    )
    deletable = models.BooleanField(
        _("deletable"), default=True, help_text=_("If False, user can't delete record.")
    )
    duplicate_entry = models.BooleanField(
        _("mutiple submissions"),
        default=False,
        help_text=_("If True, user can resubmit."),
    )
    cancel_button = models.BooleanField(
        _("Show cancel button"),
        default=False,
        help_text=_("If True, a button to cancel the survey is displayed."),
    )
    cycle_survey = models.BooleanField(
        _("Cycle the survey"),
        default=False,
        help_text=_("If True, success page returns to survey"),
    )
    private_response = models.BooleanField(
        _("private response"),
        default=False,
        help_text=_("If True, only admin and owner can access."),
    )
    can_anonymous_user = models.BooleanField(
        _("anonymous submission"),
        default=False,
        help_text=_("If True, user without authentatication can submit."),
    )
    notification_to = models.TextField(
        _("Notification To"),
        blank=True,
        null=True,
    ...
    survey_selection = models.ForeignKey(
        "SurveySelection",
    ...
    gdpr_compliant = models.BooleanField(
        ...
        default=False,
        help_text=_(
            "If True, the survey will include a unique identifier for GDPR compliance."
        ),
    )
    show_on_index = models.BooleanField(
        _("Show on index page"),
        default=False,
        help_text=_("If True, the survey will be listed on the index page."),
    )
    fullscreen = models.BooleanField(
        _("Show fullscreen"),
        default=False,
        help_text=_("If True then header/navbar etc. is removed."),
    )
    private = models.BooleanField(
        _("Private Survey"),
        default=False,
        help_text=_(
            "If True the survey will be "
    ...
    bookmark = models.BooleanField(
        ...
        default=False,
        help_text=_(
            "If True a link to the survey will be "
    ...
    def get_absolute_url(self):
        ...
        return reverse(
            "djf_surveys:respond",
            kwargs={"slug": self.slug, "selection_slug": selection},
        )
...
class Question(BaseModel):
    ...
    key = models.CharField(
        ...
        blank=True,
        help_text=_(
            "Unique key for this question, fill in the blank"
    ...
    label = models.CharField(
        _("label"), max_length=500, help_text=_("Enter your question in here.")
    )
    type_field = models.PositiveSmallIntegerField(
        _("type of input field"), choices=TYPE_FIELD
    )
...
class UserAnswer(BaseModel):
    ...
    gdpr_reference = models.UUIDField(
        _("GDPR reference"), default=None, blank=True, null=True
    )
...
class Answer(BaseModel):
    ...
    value = models.TextField(
        _("value"), help_text=_("The value of the answer given by the user.")
    )
    user_answer = models.ForeignKey(
        UserAnswer, on_delete=models.CASCADE, verbose_name=_("user answer")
    )
    ...
    def get_value(self):
        if self.question.type_field == TYPE_FIELD.rating:
            ...
            return create_star(
                active_star=int(self.value) if self.value else 0,
...
class SurveySelection(BaseModel):
    ...
    show_on_index = models.BooleanField(
        _("Show on index page"),
        default=False,
        help_text=_("If True, the survey selection will be listed on the index page."),
    )
    fullscreen = models.BooleanField(
        _("Show fullscreen"),
        default=False,
        help_text=_("Can be checked in app template for removal of header/navbar etc."),
    )
    private = models.BooleanField(
        _("Private Survey Selection"),
        default=False,
        help_text=_(
            "If True the survey will be "
            "listed only on home page for user when "
