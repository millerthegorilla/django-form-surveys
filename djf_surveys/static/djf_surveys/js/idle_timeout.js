function initialise_timeout() {
    ...
    const idleDurationSecs = parseInt(document.getElementById('link_back_on_cancel').dataset.timeout); // Use this line to change the total time a user has until redirected
    const countdownDurationSecs = parseInt(document.getElementById('link_back_on_cancel').dataset.dialogTimeout);  // Use this line to how many seconds should remain for your pop-up to come up
    const redirectUrl = encodeURI(document.getElementById('link_back_on_cancel').dataset.link);  // ADD URL TO WHICH YOU WANT TO REDIRECT USERS
