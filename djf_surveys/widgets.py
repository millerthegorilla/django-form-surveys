    ...
    def get_context(self, name, value, attrs):
        ...
        if context["widget"]["value"]:
            context["widget"]["choice_value"] = [
                x.strip() for x in context["widget"]["value"].split(",")
            ]
        else:
            context["widget"]["choice_value"] = []

        choices_count = len(context["widget"]["choice_value"])
        context["widget"]["extra"] = range(
            1 + choices_count, self.extra + 1 + choices_count
        )
        return context
...
class InlineChoiceSelectField(forms.HiddenInput):
    ...
    def get_context(self, name, value, attrs):
        context = super().get_context(name, value, attrs)
        surveys_with_selection = Survey.objects.annotate(
            surveyselection_count=Count("survey_selection")
        )
        ...
        context["widget"]["surveys"] = (
            SurveySelection.objects.get(id=self.attrs["instance"]).surveys.all()
            if self.attrs.get("instance", None)
            else None
        )
        context["widget"]["extra"] = range(
            1 + survey_count, self.extra + 1 + survey_count
        )
        return context
