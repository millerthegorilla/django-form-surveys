...
SURVEY_MASTER_TEMPLATE = (
    settings.SURVEY_MASTER_TEMPLATE
    if hasattr(settings, "SURVEY_MASTER_TEMPLATE")
    else "djf_surveys/master.html"
)

# replace admin master template
SURVEY_ADMIN_MASTER_TEMPLATE = (
    settings.SURVEY_ADMIN_MASTER_TEMPLATE
    if hasattr(settings, "SURVEY_ADMIN_MASTER_TEMPLATE")
    else "djf_surveys/admins/master.html"
)

# include navbar
SURVEY_INCLUDE_NAVBAR = (
    settings.SURVEY_INCLUDE_NAVBAR
    if hasattr(settings, "SURVEY_INCLUDE_NAVBAR")
    else True
)

# profile photo path
SURVEY_USER_PHOTO_PROFILE = (
    settings.SURVEY_USER_PHOTO_PROFILE
    if hasattr(settings, "SURVEY_USER_PHOTO_PROFILE")
    else ""
)
...
# charjs source
CHART_JS_SRC = (
    settings.CHART_JS_SRC
    if hasattr(settings, "CHART_JS_SRC")
    else '<script src="https://cdn.jsdelivr.net/npm/chart.js"></script>'
)

# number of pagination
number_of_pagination = {"survey_list": 12, "answer_list": 12}
...
SURVEY_ANONYMOUS_VIEW_LIST = (
    settings.SURVEY_ANONYMOUS_VIEW_LIST
    if hasattr(settings, "SURVEY_ANONYMOUS_VIEW_LIST")
    else False
)

# email address which the notification is sent
SURVEY_EMAIL_FROM = (
    settings.SURVEY_EMAIL_FROM if hasattr(settings, "SURVEY_EMAIL_FROM") else False
)

# to set link back button on success page
SURVEY_LINK_BACK_ON_SUCCESS_PAGE = (
    settings.SURVEY_LINK_BACK_ON_SUCCESS_PAGE
    if hasattr(settings, "SURVEY_LINK_BACK_ON_SUCCESS_PAGE")
    else "/"
)
...
if hasattr(settings, "SURVEY_DEFAULT_THANK_YOU_MESSAGE"):
    SURVEY_DEFAULT_THANK_YOU_MESSAGE = settings.SURVEY_DEFAULT_THANK_YOU_MESSAGE
else:
    SURVEY_DEFAULT_THANK_YOU_MESSAGE = _(
...
# timeout in seconds
SURVEY_FORM_TIMEOUT = (
    settings.SURVEY_FORM_TIMEOUT if hasattr(settings, "SURVEY_FORM_TIMEOUT") else 120
)

# number of seconds to show timeout dialog
SURVEY_FORM_TIMEOUT_DIALOG = (
    settings.SURVEY_FORM_TIMEOUT_DIALOG
    if hasattr(settings, "SURVEY_FORM_TIMEOUT_DIALOG")
    else 10
)
...
SURVEY_GDPR_REFERENCE_MESSAGE = (
    settings.SURVEY_GDPR_REFERENCE_MESSAGE
    if hasattr(settings, "SURVEY_GDPR_REFERENCE_MESSAGE")
